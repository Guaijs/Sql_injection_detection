...
NAME, VERSION, AUTHOR, LICENSE = "SQL injection vulnerability detection procedures", "test", "AYY", "just for test"
...
DBMS_ERRORS = {                                                                     # regular expressions used for DBMS recognition based on error message response
    ...
    "Microsoft SQL Server": (r"Driver.* SQL[\-\_\ ]*Server",
                             r"OLE DB.* SQL Server",
            ...
            for match in re.finditer(r"((\A|[?&])(?P<parameter>[^_]\w*)=)(?P<value>[^&#]+)", current):
                ...
                for (dbms, regex) in ((dbms, regex) for dbms in DBMS_ERRORS for regex in DBMS_ERRORS[dbms]):
                    if not vulnerable and re.search(regex, content[HTML], re.I) and not re.search(regex, original[HTML], re.I):
                        print(" (i) %s parameter '%s' appears to be error SQL injection vulnerable (%s)" % (phase, match.group("parameter"), dbms))
                ...
                for prefix, boolean, suffix, inline_comment in itertools.product(PREFIXES, BOOLEAN_TESTS, SUFFIXES, (False, True)):
                    if not vulnerable:
                        ...
                        if vulnerable:
                            print(" (i) %s parameter '%s' appears to be blind SQL injection vulnerable (e.g.: '%s')" % (phase, match.group("parameter"), payloads[True]))
                            retval = True
        if not usable:
